...
如何去衡量一款应用的质量好坏？这一问题延伸出`APM`这种目的性极强的工具化开发方向。尽管`APM`中存在着诸多指标用来衡量应用不同的性能与质量，但对于公司来说，`crash`是唯一不可或缺的采集指标，一套完善的`crash`采集方案能够的避免因为质量问题导致用户流失。然而，如果`crash`在采集过程中出现了异常丢失，那么这对于开发者来说可能是一场噩梦。
